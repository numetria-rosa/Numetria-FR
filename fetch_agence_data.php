...
if ($result && $result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<tr>
            ...
            <td>{$created_on}</td>
            <td>{$credit}</td>
            <td>{$etatText}</td> <!-- Display Activé or Désactivé -->
            <td><button class='btn btn-warning' onclick=\"location.href='reglement.php?pid={$pid}'\">Réglements</button></td>
            <td><button class='btn btn-info' onclick=\"location.href='reservation_agence.php?pid={$pid}'\">Réservations</button></td> <!-- Added the Reservations button -->
        </tr>";
    }
} else {
    // No data found
